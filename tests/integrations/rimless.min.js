...
function connect(schema, options) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        function handleHandshakeResponse(event) {
            ...
            var close = function () {
                self.removeEventListener(types_1.events.MESSAGE, handleHandshakeResponse);
                unregisterRemote();
                unregisterLocal();
            };
            connected = true;
        ...
        self.addEventListener(types_1.events.MESSAGE, handleHandshakeResponse);
        var payload = {
...
},{"./helpers":2,"./rpc":5,"./types":6}],2:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.CONNECTION_TIMEOUT = 1000;
/**
 * check if the remote is trusted
 *
 * @param event
 */
function isTrustedRemote(event) {
    // TODO: implement
    return true;
}
exports.isTrustedRemote = isTrustedRemote;
/**
...
function getOriginFromURL(url) {
    ...
    if (regexResult) {
        ...
        _a = regexResult[1], protocol = _a === void 0 ? location.protocol : _a, hostname = regexResult[2], port = regexResult[4];
    }
    else {
    ...
    var portSuffix = port && port !== ports[protocol] ? ":" + port : "";
...
function isValidTarget(iframe, event) {
    var childURL = iframe.getAttribute("src");
    var childOrigin = helpers_1.getOriginFromURL(childURL);
    var hasProperOrigin = event.origin === childOrigin;
    var hasProperSource = event.source === iframe.contentWindow;
 ...
 * @returns Promise
 */
function connect(guest, schema, options) {
    if (schema === void 0) { schema = {}; }
    if (!guest)
        throw new Error("a target is required");
    // this check should be improved
    var guestIsWorker = guest.onerror !== undefined &&
        guest.onmessage !== undefined;
    var listeners = guestIsWorker ? guest : window;
    return new Promise(function (resolve, reject) {
        var connectionID = short_uuid_1.default.generate();
        // on handshake request
        function handleHandshake(event) {
            if (!guestIsWorker && !isValidTarget(guest, event))
                return;
            if (event.data.action !== types_1.actions.HANDSHAKE_REQUEST)
                return;
            // register local methods
            var localMethods = helpers_1.extractMethods(schema);
            var unregisterLocal = rpc_1.registerLocalMethods(schema, localMethods, connectionID, guestIsWorker ? guest : undefined);
            // register remote methods
            var _a = rpc_1.registerRemoteMethods(event.data.schema, event.data.methods, connectionID, event, guestIsWorker ? guest : undefined), remote = _a.remote, unregisterRemote = _a.unregisterRemote;
            var payload = {
                action: types_1.actions.HANDSHAKE_REPLY,
            ...
            // close the connection and all listeners when called
            var close = function () {
                listeners.removeEventListener(types_1.events.MESSAGE, handleHandshake);
                unregisterRemote();
                unregisterLocal();
            };
            // resolve connection object
        ...
        listeners.addEventListener(types_1.events.MESSAGE, handleHandshake);
    });
}
exports.default = ({
    connect: connect,
...
},{"./helpers":2,"./rpc":5,"./types":6,"short-uuid":11}],4:[function(require,module,exports){
"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
var guest_1 = __importDefault(require("./guest"));
exports.guest = guest_1.default;
var host_1 = __importDefault(require("./host"));
exports.host = host_1.default;

},{"./guest":1,"./host":3}],5:[function(require,module,exports){
"use strict";
    ...
    methods.forEach(function (methodName) {
        ...
        function handleCall(event) {
            return __awaiter(this, void 0, void 0, function () {
                var _a, action, callID, connectionID, callName, _b, args, payload, result, error_1;
                return __generator(this, function (_c) {
                    switch (_c.label) {
                        case 0:
                            _a = event.data, action = _a.action, callID = _a.callID, connectionID = _a.connectionID, callName = _a.callName, _b = _a.args, args = _b === void 0 ? [] : _b;
                            if (action !== types_1.actions.RPC_REQUEST)
                                return [2 /*return*/];
                            if (!helpers_1.isTrustedRemote(event))
                                return [2 /*return*/];
                            if (!callID || !callName)
                                return [2 /*return*/];
                            ...
                            if (connectionID !== _connectionID)
                                return [2 /*return*/];
                            payload = {
                                action: types_1.actions.RPC_RESOLVE,
                                callID: callID,
                                callName: callName,
                                connectionID: connectionID,
                                error: null,
                        ...
                        case 1:
                            _c.trys.push([1, 3, , 4]);
                            return [4 /*yield*/, lodash_get_1.default(schema, methodName).apply(void 0, args)];
                        case 2:
                            result = _c.sent();
                        ...
                        case 3:
                            error_1 = _c.sent();
                            payload.error = JSON.parse(JSON.stringify(error_1, Object.getOwnPropertyNames(error_1)));
                            return [3 /*break*/, 4];
                        case 4:
                            if (guest)
                                guest.postMessage(payload);
                            else if (helpers_1.isWorker())
                                self.postMessage(payload);
                            else
                                event.source.postMessage(payload, event.origin);
                            return [2 /*return*/];
        ...
        if (guest)
            guest.addEventListener(types_1.events.MESSAGE, handleCall);
        else
            self.addEventListener(types_1.events.MESSAGE, handleCall);
        listeners.push(function () { return self.removeEventListener(types_1.events.MESSAGE, handleCall); });
    });
    return function () { return listeners.forEach(function (unregister) { return unregister(); }); };
}
exports.registerLocalMethods = registerLocalMethods;
/**
...
function createRPC(_callName, _connectionID, event, listeners, guest) {
    ...
    return function () {
        ...
        return new Promise(function (resolve, reject) {
            var callID = short_uuid_1.default.generate();
            // on RPC response
            function handleResponse(event) {
                var _a = event.data, callID = _a.callID, connectionID = _a.connectionID, callName = _a.callName, result = _a.result, error = _a.error, action = _a.action;
                if (!helpers_1.isTrustedRemote(event))
                    return;
                if (!callID || !callName)
                    return;
                ...
                if (action === types_1.actions.RPC_RESOLVE)
                    return resolve(result);
                if (action === types_1.actions.RPC_REJECT)
                    return reject(error);
            }
            // send the RPC request with arguments
            var payload = {
                action: types_1.actions.RPC_REQUEST,
            ...
            if (guest)
                guest.addEventListener(types_1.events.MESSAGE, handleResponse);
            else
                self.addEventListener(types_1.events.MESSAGE, handleResponse);
            listeners.push(function () { return self.removeEventListener(types_1.events.MESSAGE, handleResponse); });
            if (guest)
                guest.postMessage(payload);
            else if (helpers_1.isWorker())
                self.postMessage(payload);
...
function registerRemoteMethods(schema, methods, _connectionID, event, guest) {
    if (schema === void 0) { schema = {}; }
    if (methods === void 0) { methods = []; }
    var remote = __assign({}, schema);
    var listeners = [];
    methods.forEach(function (methodName) {
        var rpc = createRPC(methodName, _connectionID, event, listeners, guest);
        lodash_set_1.default(remote, methodName, rpc);
    });
    return { remote: remote, unregisterRemote: function () { return listeners.forEach(function (unregister) { return unregister(); }); } };
}
exports.registerRemoteMethods = registerRemoteMethods;

},{"./helpers":2,"./types":6,"lodash.get":9,"lodash.set":10,"short-uuid":11}],6:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
var events;
(function (events) {
...
},{"any-base":7,"uuid/v4":14}],12:[function(require,module,exports){
/**
...
},{"./lib/bytesToUuid":12,"./lib/rng":13}]},{},[4])(4)
});
